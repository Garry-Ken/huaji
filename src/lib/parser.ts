...
function cnToNum(s: string): number {
  let result = 0, sub = 0, current = 0, lastUnit = 0, sawZero = false
  for (const ch of s) {
    if (ch in CN_DIGITS) {
  ...
  let total = result + sub
  // 口语省略末位单位："三百八"=380、"两千三"=2300、"三万五"=35000；"三百零八"=308 不缩放
  if (current > 0) {
    total += lastUnit >= 10 && !sawZero ? current * (lastUnit / 10) : current
  }
  return total
}
...
const INCOME_KW = new RegExp([
  ...
  '补贴', '补助', '津贴',
  // 其他
  '中奖', '彩票', '中了.*[彩奖]', '收到红包', '抢到红包', '收钱', '收款', '收到款', '转账收入', '众筹', '打赏收入',
  // 卖出
  '卖了', '卖出', '卖掉', '出手',
  // 打赏收入（别人打赏我）
  '打赏', '收到打赏',
...
// 明显的新消费动作（交通/网购/缴费/采买）作为新一笔的开头
const ACTIVITY_START = /^(?:打车|打的|打了个?车|地铁|公交|坐车|加油|停车|淘宝|京东|拼多多|网购|充值|充了|交了?|缴|付了?|买了?|还了?[^我])/
...
export function parseExpense(rawText: string, now = new Date()): ParseResult {
  ...
  // 收入/借贷不提取菜名明细（标题走收入词典/对方），避免把"工资到账"等当成菜
  const items = txType === 'income' || isDebt ? [] : extractFoodNames(text, foodHits, location, merchant)
...
const FOOD_LEADING_RE = /^(?:今天|昨天|前天|明天|早上|早晨|上午|中午|下午|午后|傍晚|晚上|今晚|昨晚|凌晨|和|跟|与|及|在|去|到|的|了|还|又|再|顿|份|聚餐|聚会|早茶|早点|早饭|午饭|中饭|晚饭|正餐|大餐|宵夜|夜宵|加餐|下午茶|办酒席|请客|请|约了?|和家人|和朋友|和同事|跟家人|跟朋友|跟同事|吃了?顿?饭?|喝了?水?|点了?|要了?|买了?|来了?|整了?|搞了?|煮了?|做了?|烤了?|炸了?|蒸了?|炒了?|啃了?|嗑了?|嚼了?|[一二两三四五六七八九十几半\d]+\s*(?:份|碗|杯|个|只|盒|袋|瓶|块|片|盘|包|桶|罐|条|把|颗|粒|串|根|口|桌|顿))+/
...
// 场景/方式词：不是具体菜名（如"外卖/食堂/聚餐"）
const DISH_STOP = new Set(['外卖', '食堂', '快餐', '工作餐', '聚餐', '聚会', '夜宵', '宵夜', '下午茶', '大餐', '正餐', '加餐', '堂食', '自助', '请客', '办酒席'])
...
function extractFoodNames(
  ...
  if (segments.length > 1) {
    ...
    for (const seg of segments) {
      ...
      if (DISH_STOP.has(clean)) continue          // "外卖/食堂"等场景词不是菜
      names.push(clean)
    }
  ...
  if (foodHits.length > 0) {
    ...
    for (const f of foodHits) {
      ...
      for (const k of f.match) {
        ...
        if (kl === merchantLower) continue // 商家名(如"星巴克")不当菜名，优先取真正的饮品
        if (DISH_STOP.has(k)) continue // "外卖/食堂"等场景词不是菜
        if (lower.includes(kl) && k.length > best.length) best = k
      }
